...
const Map = (props) => {
  ...
  const scale = chroma.scale(themeStr ?
    themeStr.values :
  ...
  useLayoutEffect(() => {
    ...
    polygonSeries.mapPolygons.template.adapter.add('fill', function(
      ...
      if (mapPolygon.dataItem.values['value'].workingValue <= 5) {
        return am4core.color(colors[0]);
      } else if (mapPolygon.dataItem.values['value'].workingValue <= 15) {
        return am4core.color(colors[4]);
        // return am4core.color('#FFCC33'); // lightyellow
      } else if (mapPolygon.dataItem.values['value'].workingValue > 15) {
        return am4core.color(colors[9]);
      } else {
  ...
  useLayoutEffect(() => {
    ...
    mapData.forEach( (item)=>{
      item.percentage = item.value ? (item.value).toFixed(2) + ' cases/1000' : '';
    });
