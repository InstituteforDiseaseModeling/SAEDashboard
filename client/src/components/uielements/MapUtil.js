 ...
 * @param {*} rainFallStations
 * @param {*} layerControl
 * @param {*} formatMessage
 * @param {*} clickHandler
 */
export function addRainfallStations(rainFallStations, layerControl, formatMessage, clickHandler) {
  const L = require('leaflet');
  ...
  for (const station in rainFallStations) {
    if (station) {
      ...
      const marker = L.marker([parseFloat(_station.Lat_y), parseFloat(_station.Long_x)],
          {icon: rainMarker(_station.Strata.toLowerCase())})
